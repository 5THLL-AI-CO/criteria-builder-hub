...
const AgentCardES = ({
  category, agentName, initials, professional, description, calibration, status, isComingSoon = false, onClick,
}: AgentCardESProps) => {
  return (
    <div
      onClick={onClick}
      className={cn(
        ...
        isComingSoon ? "hover:border-accent/20 hover:opacity-60" : "hover:border-accent/30 hover:-translate-y-0.5"
      )}
    >
      {isComingSoon && (
      ...
      <div className={cn(isComingSoon && "opacity-45")}>
        ...
        <h3 className="font-display text-xl md:text-2xl font-semibold text-text-primary mt-4">{agentName}</h3>
        ...
        <p className="text-text-secondary text-base mt-4 line-clamp-2">{description}</p>

        <div className="border-t border-criteria-border pt-4 mt-auto flex justify-between items-center">
          <span className="font-mono text-sm text-text-secondary">Calibración: {calibration}</span>
