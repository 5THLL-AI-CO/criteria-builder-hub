...
const AgentDetailModal = ({ isOpen, onClose, agent }: AgentDetailModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("https://formspree.io/f/REPLACE_REQUEST_FORM_ID", {
        ...
        body: JSON.stringify({
          ...formData,
          _agent: agent.agentName,
          _type: "consultation-request",
        }),
      });
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-background/85 flex items-center justify-center p-4 overflow-y-auto"
      onClick={handleBackdropClick}
    >
      <div
        className="relative bg-surface border border-criteria-border rounded-2xl p-8 md:p-12 max-w-4xl w-full my-8 animate-scale-in"
      ...
      >
        ...
        <div className="grid md:grid-cols-[1.5fr_1fr] gap-8 md:gap-12">
          ...
          <div>
            ...
            {/* Agent name */}
            <h2 className="font-display text-2xl md:text-3xl font-bold text-text-primary mt-6">
              {agent.agentName}
            </h2>

            {/* Description */}
            <p className="text-text-secondary text-base mt-3 max-w-xl">
              {agent.description}
            </p>

            {/* What this agent does */}
            <p className="text-text-tertiary text-xs font-medium uppercase tracking-wider mt-8">
              What this agent does
            </p>
            ...
            <div className="flex flex-col gap-2 mt-4">
              {agent.steps.map((step, index) => (
                <div
                  key={index}
                  className="bg-background border border-criteria-border rounded-lg p-4 flex gap-3"
                >
          ...
          <div className="bg-background border border-criteria-border rounded-xl p-6 md:p-8">
            ...
            {isSubmitted ? (
              ...
              <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
                  ...
                  <div key={field.name}>
                    ...
                    {field.type === "textarea" ? (
                      <textarea
                        ...
                        className={cn(
                          "w-full bg-surface border border-criteria-border rounded-lg px-4 py-3 text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent resize-y",
                      ...
                      <input
                        ...
                        onChange={(e) => handleInputChange(field.name, e.target.value)}
                        className={cn(
                          "w-full bg-surface border border-criteria-border rounded-lg px-4 py-3 text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent",
                          errors[field.name] && "border-destructive"
                        )}
                      />
                    )}
                ...
                >
                  {isSubmitting ? "Submitting..." : agent.ctaText}
                </button>

                <p className="text-text-tertiary text-xs text-center">
                  Output delivered within 1 hour. Your data is handled securely.
                </p>
              </form>
            )}
          </div>
